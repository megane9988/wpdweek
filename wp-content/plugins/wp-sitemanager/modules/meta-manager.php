...
private function get_meta() {
	...
	if ( is_home() && ! is_front_page() && ( get_query_var( 'pagename' ) || get_query_var( 'page_id' ) ) ) {
		$post_id = get_option( 'page_for_posts' );
		if ( $post_id ) {
			$page_for_posts = get_post( $post_id );
			if ( $page_for_posts ) {
				if ( $page_for_posts->_keywords ) {
					$option['keywords'] = $page_for_posts->_keywords;
				}
				if ( $page_for_posts->_description ) {
					$option['description'] = $page_for_posts->_description;
				}
			}
		}
	} elseif ( is_singular() ) {
